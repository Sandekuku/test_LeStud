...
router.post("/", passport.authenticate("user", { session: false }), async (req, res) => {
  try {
    ...
    const user = await UserObject.create({ name: username, password, organisation: req.user.organisation});
...
router.get("/", passport.authenticate("user", { session: false }), async (req, res) => {
  try {
    const users = await UserObject.find({organisation: req.user.organisation }).sort("-last_login_at");
    return res.status(200).send({ ok: true, data: users });
  } catch (error) {
    console.log(error);
    res.status(500).send({ ok: false, code: SERVER_ERROR, error });
